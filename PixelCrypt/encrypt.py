...
def unswap_pixels(pixels,indices): #this function is essentially used for decryption, ,it takes the encrypted pixel array pixels that has its pixels shuffled and orginal shuffle indices which records how the pixels were rearranged and it reconstructs the original image by undoing the shuffle
    flat=pixels.reshapr(-1,3) #flattens the 3D array of RGM into a 2D array. each row is now a pixel with RGB values.
...
def xor_pixels(pixels,key=99): #thisfunction applies a simple XOR bitwise operation to each pixel using a key. it is a basic encryption technique that flips bits based on a key. it is used to further encrypt the pixel values after shuffling.
